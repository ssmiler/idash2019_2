...
parser.add_argument('--pred_file', type=str, required=True,
                    help='predictions file (.csv')
parser.add_argument('--target_file', type=str,
                    required=True, help='target file (.csv')
...
def read_and_transform(inp_file):
    # read input csv file and transpose
    df = pd.read_csv(inp_file, sep='\t', header=None).T
...
print("Micro-AUC score: {}".format(sklearn.metrics.roc_auc_score(df_target,
                                                                 df_pred, average='micro')))

print("MAP score: {}".format(np.mean(mpreds == mtargets)))

t = ((mtargets != 0)).sum(axis=1)
print("MAP non-ref score: {}".format(np.mean(((mtargets != 0) &
                                              (mpreds == mtargets)).sum(axis=1)[t != 0] / t[t != 0])))
